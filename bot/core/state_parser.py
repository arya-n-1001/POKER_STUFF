...
def parse_state(round_state, hole_card, player_uuid, valid_actions):
    ...
    # determine hero position index
    dealer_btn = round_state["dealer_btn"]

    # --- active seats only (stack > 0) ---
    active_seats = [
        s for s in seats
        if s["stack"] > 0
    ]

    # --- rotate table so BTN is first ---
    n = len(seats)
    ...
    # --- hero index relative to ACTIVE table ---
    relative_pos = next(
    ...
    alive_seats = [s for s in seats if s["state"] == "participating"]
    n = len(seats)
